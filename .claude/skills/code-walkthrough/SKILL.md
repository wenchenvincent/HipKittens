...
- **`GFX_ACTIVITY` can underreport for short-kernel burst patterns.** Caught this when measuring the FP8 4-wave kernel: each launch is ~0.43 ms, the warmup phase queues 500 of them back-to-back (~215 ms of continuous work). `amd-smi`'s `GFX_ACTIVITY` reported `0%` throughout, but `SOCKET_POWER` rose from 245 W idle → 297 W active and `CLK` rose from 157 MHz → 2400 MHz — clear active-state signatures the activity metric missed. **Use power and clock as primary active-state indicators; treat `GFX_ACTIVITY` as a hint, not a truth.** When in doubt, cross-check the clock with `rocm-smi -d N --showclocks` (different tool, independent sampling path) — should agree.
