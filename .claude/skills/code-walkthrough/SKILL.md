...
- **`amd-smi` metrics are ALL unreliable for short-kernel burst patterns.** Three separate readings led me astray on the FP8 4-wave kernel before the reviewer caught each one:
  - **`GFX_ACTIVITY`** read `0%` during the active phase (activity counter sampling lagged the burst pattern).
  - **`CLK` / `rocm-smi --showclocks`** read 2400 MHz (the SCLK setpoint DPM boosts to between launches), not the achieved compute clock (~1.5 GHz).
  - **`SOCKET_POWER`** read ~297 W (the GPU's between-launch quiescent power), not the sustained compute power (~1100 W avg, peaks 1349 W).

  All three metrics give the wrong answer for the same root cause: the default bench's 258 ms of GPU work (500 warmup × 0.43 ms + 100 timing × 0.43 ms) is shorter than DPM transition windows AND shorter than amd-smi's internal counter settle times. The kernel finishes before any of these readings reach steady-state.
- **The fix: do a long-warmup measurement (15+ seconds of sustained GPU work)** before quoting any amd-smi-derived number. For HK's bench harnesses, that means bumping `warmup_iters` from 500 to ~30000 so the GPU is genuinely loaded when sampling begins. Re-bench with the longer warmup; compare to the short bench. If sustained numbers differ from short numbers (they will), the short numbers are the artifact and the long numbers are the truth.
- **Cross-check the clock independently via cycle counts.** ATT records each wave's `duration` in sclk cycles. With known occupancy and grid you can compute per-workgroup wall time as `kernel_ms / (num_workgroups / num_CUs)`, then `clock = wave_duration / workgroup_wall_time`. This bypasses the entire amd-smi reporting layer.
- **The diagnostic to watch for: per-cycle efficiency that looks much worse than a comparable kernel's.** When I had the FP8 kernel at "51% of peak" but BF16 was at "78%," that gap was the signal the clock measurement was wrong — same scheduling family, similar code, shouldn't differ by 27 percentage points. Reaching for the cycle-counter derivation (or the long-warmup bench) at that moment, instead of accepting the amd-smi number, would have caught all three measurement errors at once.
